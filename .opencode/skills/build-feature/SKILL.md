...
Folder name: **camelCase, singular noun** matching the domain term (`users`, `orders`, `invoices`). Pluralise only if the domain already uses the plural (`users` stays plural because the term is _the Users Directory_).
